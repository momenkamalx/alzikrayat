...
#Return True if extension is (jpg/jpeg/png)
def allowed_file(filename):
    return "." in filename and filename.rsplit(".", 1)[1].lower() in ALLOWED_EXT

"""
    Validate and save an uploaded photo
    Checks: title length, file present, file type, file size.
    Save the file in a random uuid filename and inserts a DB row.
    Returns (yrue or none) on success or (False or error_message) on fail.
"""
...
"""
    Delete a photo file and DB row  only if the user owns it
    Returns True if deleted or false if not found or not owned
"""
